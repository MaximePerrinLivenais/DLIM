...
class Discriminator(keras.Model):
    def __init__(self, filters = 64, kernel_initializer = keras.initializers.RandomNormal(mean = 0.0, stddev = 0.02),
                    nb_downsampling_blocks = 3, name = None):
        super(Discriminator, self).__init__()
        input_tensor = layers.Input(shape = input_img_size, name = name + '_img_input')
